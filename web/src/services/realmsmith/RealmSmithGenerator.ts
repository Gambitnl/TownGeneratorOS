...
export class TownGenerator {
    ...
    private decorateDeadEnds(tiles: Tile[][]) {
        for (let x = 1; x < WIDTH - 1; x++) {
            for (let y = 1; y < HEIGHT - 1; y++) {
                if (tiles[x][y].type === TileType.ROAD_DIRT) {
                    // Check neighbors
                    let connections = 0;
                    if (tiles[x + 1][y].type.includes('ROAD')) connections++;
                    if (tiles[x - 1][y].type.includes('ROAD')) connections++;
                    if (tiles[x][y + 1].type.includes('ROAD')) connections++;
                    if (tiles[x][y - 1].type.includes('ROAD')) connections++;
                    ...
                    if (connections === 1) {
                        ...
                        if (this.rng.chance(0.4)) {
                            tiles[x][y].doodad = {
                                type: this.rng.pick([DoodadType.CRATE, DoodadType.WELL, DoodadType.STREET_LAMP]),
                                id: `de_${x}_${y}`,
                                offsetX: 0, offsetY: 0
                            };
                        }
    ...
    private placeDoodads(tiles: Tile[][]) {
        ...
        for (let x = 0; x < WIDTH; x++) {
            for (let y = 0; y < HEIGHT; y++) {
                const t = tiles[x][y];
                if (t.buildingId || t.doodad || t.type.includes('ROAD') || t.type === TileType.WALL || t.type === TileType.FARM) continue;

                // Only place on valid ground
                const validGround = [TileType.GRASS, TileType.DIRT, TileType.SNOW, TileType.SAND, TileType.MUD, TileType.ASH, TileType.ROCK_GROUND, TileType.CRYSTAL_FLOOR];
                if (!validGround.includes(t.type)) continue;

                const noiseVal = this.noise.noise(x * 0.15, y * 0.15);

                if (noiseVal > (1.0 - treeDensity)) {
                    if (this.rng.chance(0.6)) {
                        t.doodad = {
    ...
    private placeStreetLamps(tiles: Tile[][]) {
        // Place lamps at intersections or along main roads
        for (let x = 2; x < WIDTH - 2; x += 3) {
            for (let y = 2; y < HEIGHT - 2; y += 3) {
                if (tiles[x][y].type === TileType.ROAD_MAIN) {
                    if (this.rng.chance(0.15)) {
                        // Find empty spot next to road
                        const neighbors = [{ x: 1, y: 0 }, { x: -1, y: 0 }, { x: 0, y: 1 }, { x: 0, y: -1 }];
                        for (const n of neighbors) {
                            const tx = x + n.x;
                            const ty = y + n.y;
                            const t = tiles[tx][ty];
                            if (!t.type.includes('ROAD') && !t.buildingId && !t.doodad && t.type !== TileType.WALL && t.type !== TileType.WATER_DEEP) {
                                t.doodad = {
