...
# Discord Bot API Functions - Claude Code Specific
# Source this file: source discord-bot-functions-claude.sh
...
# Ensure the current bot identity matches ClaudeCLI
ensure_bot_identity() {
    local expected="ClaudeCLI"
    ...
    if [ "$who" != "$expected" ]; then
        ...
        echo "💡 Make sure you're using: cp .env.claude .env" >&2
        return 1
    fi
    return 0
}

# Send message to Discord channel via Bot API (Claude-specific with identity guard)
discord_send_message() {
    local channel_id="$1"
    local message="$2"
    local username="${3:-Claude Code}"
    
    # Guard: ensure we are posting as ClaudeCLI
...
check_file_conflicts() {
    ...
    conflicts=$(discord_read_messages "$DISCORD_FILE_RESERVATIONS_CHANNEL" 50 | grep "FILE_RESERVE.*$file_path" | grep -v "Claude Code")
